...
from icon_registration import network_wrappers

def to_floats_generic(args, stats):
    '''
        Combine all logging functions
    '''
    if args.training_type == 'basic':
        return to_floats_basic(stats)
    elif args.training_type == 'basic+':
        return to_floats(stats)
    elif args.training_type == 'unsupervised':
        return to_floats_can_unsupervised(args, stats)
    else:
        return to_floats_can(args, stats)

def to_floats_basic(stats):
    out = []
    for v in stats:
        if isinstance(v, torch.Tensor):
            v = torch.mean(v).cpu().item()
        out.append(v)
    return BASICLoss(*out)
...
def to_floats_can_unsupervised(args, stats):
    out = []
    for v in stats:
        if isinstance(v, torch.Tensor):
            v = torch.mean(v).cpu().item()
        out.append(v)
    if args.log_mono:
        return ICONLoss_can_mono_unsupervised(*out)
    return ICONLoss_can_unsupervised(*out)

BASICLoss = namedtuple(
    "BASICLoss",
    "all_loss similarity_loss",
)

ICONLoss = namedtuple(
    "ICONLoss",
    "all_loss similarity_loss inverse_consistency_loss transform_magnitude flips",
)
...
ICONLoss_can_unsupervised = namedtuple(
    "ICONLoss_can",
    "all_loss similarity_loss inverse_consistency_loss canonical_consistency_loss transform_magnitude flips",
)

ICONLoss_can_mono_unsupervised = namedtuple(
    "ICONLoss_can",
    "all_loss similarity_loss mono_similarity_loss inverse_consistency_loss canonical_consistency_loss transform_magnitude flips",
)
...
class SSD(SimilarityBase):
    ...
    def __call__(self, image_A, image_B):
        assert image_A.shape == image_B.shape, f"The shape of image_A {image_A.shape} and image_B {image_B.shape} sould be the same."
...
class MINDSSC(SimilarityBase):
    def __init__(self, radius=2, dilation=1):
        super().__init__(isInterpolated=False)
        self.radius = radius
